    ...
    def batch_(self, rpc_calls):
        ...
        for rpc_call in rpc_calls:
            ...
            batch_data.append({"jsonrpc": "2.0", "method": m,
                               "params": rpc_call, "id": AuthServiceProxy.__id_count})
...
#############################################################################################################
#que3 = Queue()                                                                                              #
#t7 = Thread(target = lambda q, arg1: q.put(host.call(arg1)), args=(que3, 'getblockcount'))                  #
#t7 = Thread(target = lambda : AuthServiceProxy("http://%s:%s@127.0.0.1:8332"%('iscal', 'marenostrum')))                  #
# t7.start()                                                                                                  #   threading
#t7.join()                                                                                                   #
#rpc_connection = que3.get()                                                                                         #
#############################################################################################################
#time.sleep (2)
rpc_connection = AuthServiceProxy(
    "http://%s:%s@127.0.0.1:8332" % ('iscal', 'marenostrum'))
